...
class InDimDynamicLinearSparseFunction(torch.autograd.Function):
    @staticmethod
    def forward(ctx,
                data,
                weight,
        ...
        return dim_dynamic_sparse_linear_cpp.indim_forward(data, weight, index, bias)
    @staticmethod
    def backward(ctx, *grad_out):
        data, weight, index = ctx.saved_tensors
        a_grad, w_grad = dim_dynamic_sparse_linear_cpp.indim_backward(data, weight, grad_out[0], index)
        return a_grad, w_grad, None, None
...
class DimDynamicLinear(torch.nn.Module):
    ...
    def forward(self, data, mask):
        ...
        if self.in_or_out == 0:
            ...
            return InDimDynamicLinearSparseFunction.apply(data, self.weight, self.bias, _pos)
    ...
    def ref_forward(self, data, mask):
        ...
        if self.in_or_out == 0:
            ...
            re = F.linear(data, torch.index_select(self.weight.t(), 1, _pos),
                     self.bias)
        return re
