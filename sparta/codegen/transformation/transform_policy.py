...
from sparta.common.abstract import SparseModuleInfo, TeSA
...
class TransformPolicyBase(abc.ABC):
    ...
    def align_bit(self, tesa: TeSA, decompose: bool = False, hardware_align: list = None):
        assert tesa.tesa.dtype == torch.int32, f"TeSA has wrong data types which should be torch.int32 but {tesa.tesa.dtype} founded"
        aligned_tesas = []
        if decompose == True:
            bits_options = torch.unique(tesa.tesa, sorted=True)
            
        else:
            max_bit = torch.max(tesa.tesa)
            new_tesa = torch.zeros_like(tesa.tesa)
            new_tesa[tesa.tesa>0] = max_bit
            aligned_tesas.append(new_tesa)
        if hardware_align is not None:
            hardware_align = sorted(hardware_align)
            for tesa in aligned_tesas:
                n_bit = torch.max(tesa)
                _pos = bisect.bisect_right(hardware_align, n_bit)
                tesa.data[tesa>0] = hardware_align[_pos]
        return aligned_tesas

class MatmulTransBlockPolicy(TransformPolicyBase):
    def transform_module(self, module_sparsity: SparseModuleInfo):
        pass


applicable_optypes = {
    MatmulTransBlockPolicy: ['aten::matmul', ]
}
    ...
    def _align_to_hardware_bits(self, bits: set, bit_options: list) -> bool:
        ...
        for bit in bits:
            ...
            if aligned_ideal_bit in bit_options:
                ...
                aligned_bits.add(
                    min([b for b in bit_options if b > aligned_ideal_bit]))
        return aligned_bits
    ...
    def bit_align(self, tesa: TeSA, n_candidates: int = 1, decompose: bool = True) -> List[tuple]:
        ...
        if not decompose or len(aligned_bits) == 1:
            ...
            if aligned_bit not in bit_options:
                aligned_bit = min(
                    [bit for bit in bit_options if bit > aligned_bit])
