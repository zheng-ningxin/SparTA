...
__global__ void BLOCK_SPARSE_MATMUL_NT_OPENAI(
    float* A,
    float* B,
    int * index,
    int GLOBAL_M,
                        ...
                        )
{
    ...
    //a_grad(M*K) = grad_c(MxN) * weight^T(NxK)
    dim3 a_grid_dim(K/BLOCK_SIZE_N, M/BLOCK_SIZE_M);
    BLOCK_SPARSE_MATMUL_NT_OPENAI<<<a_grid_dim, w_block_dim>>>(grad_out, weight, index, M, N, K, a_grad);
    // BLOCK_SPARSE_MATMUL_NN_OPENAI<<<a_grid_dim, w_block_dim>>>(grad_out, weight, a_grad, index, M, N, K);
}
