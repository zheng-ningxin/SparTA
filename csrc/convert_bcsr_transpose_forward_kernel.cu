...
__global__ void convert_bcsr_kernel_transpose_1(const int * __restrict__  mask, float * __restrict__  dense, int h, int w,
...
__global__ void convert_bcsr_kernel_transpose_2(const int * __restrict__  mask, float * __restrict__  dense, int h, int w,
    int block_h, int block_w, int * row, int *col, int * row_pos,
    float * values, int * extra_buffer, int * block_index)
{
    ...
    if (tid==0){
        ...
        if (pos_id>=0){
            ...
            ori_bx = bx;
            ori_by = extra_buffer[ 2*w + bx * gridDim.y + pos_id];       
            
            row[bx] = prefix_count;
            col[prefix_count + pos_id] = ori_by;
            row_pos[prefix_count + pos_id] = bx;
            block_index[bx * gridDim.y + ori_by] = prefix_count + pos_id;
        }
        else if(pos_id==-1){
            for(int i=0; i<bx; i++){
                prefix_count +=  extra_buffer[w+i];
    ...
    int block_w)
{
    ...
    assert(h % block_h==0);
    assert(w % block_w==0);
    torch::Tensor t_pattern = sparse_pattern.t().contiguous();
    ...
    AT_DISPATCH_FLOATING_TYPES(dense_values.type(), "convert_bcsr", ([&]
        { convert_bcsr_transpose(
                t_pattern.data_ptr<int>(),
