...
__global__ void BATCH_BLOCK_SPARSE_MATMUL_CONDENSE_DIM_M_V2(float* A_val, int* A_row, int* A_col, float*B, float*C, int GLOBAL_M, int GLOBAL_K, int GLOBAL_N, int BLOCK_H, int BLOCK_W)
{

    // does not support the batch matmul currently
    const int BLOCK_SIZE_M = 32;  // 64
    const int BLOCK_SIZE_K = 64;  //8
    const int BLOCK_SIZE_N = 32;  //128
    const int THREAD_SIZE_K = 64;
    ...
    if (index_start < index_end){
        ...
        // if(threadIdx.x == 0){
        //     printf("bx:%d by:%d bz:%d N_STEP:%d block_n_start:%d block_n_end:%d\n", bx, by, bz, N_STEP, (N_STEP/BLOCK_SIZE_N)*blockIdx.z, (N_STEP/BLOCK_SIZE_N)*(blockIdx.z+1));
        // }
        for(int block_n_id = (N_STEP/BLOCK_SIZE_N)*blockIdx.z; block_n_id < (N_STEP/BLOCK_SIZE_N)*(blockIdx.z+1); block_n_id++)
        {
            #pragma unroll
            for (int i = 0; i < 8; i++)
                for (int j = 0; j < 4; j++)
...
void openai_bmm_32_64_32_condense_dim_m_launch_v2(float* A_val, int* A_row, int* A_col, float*B, float*C, int GLOBAL_M, int GLOBAL_K, int GLOBAL_N, int BLOCK_H, int BLOCK_W, int SPARSE_VAL_SIZE, int batchsize)
{
    const dim3 dimBlock(256);
    assert(BLOCK_W==64);
    const int TILE_N = 16;
    assert(GLOBAL_N %(TILE_N * 32)==0);
    dim3 dimGrid(GLOBAL_M/32, GLOBAL_K/64, TILE_N);
    BATCH_BLOCK_SPARSE_MATMUL_CONDENSE_DIM_M_V2<<<dimGrid, dimBlock>>>(A_val, A_row, A_col, B, C, GLOBAL_M, GLOBAL_K, GLOBAL_N, BLOCK_H, BLOCK_W);
}
    ...
    int block_nnz)
{
    ...
    AT_DISPATCH_FLOATING_TYPES(B.type(), "openai_bmm_forward_dim_m_v2", ([&]
                            { 
                                openai_bmm_32_64_32_condense_dim_m_launch_v2(
                                    values.data_ptr<float>(),
                                    row_ptr.data_ptr<int>(),
                                    col_idx.data_ptr<int>(),
                                    B.data_ptr<float>(),
