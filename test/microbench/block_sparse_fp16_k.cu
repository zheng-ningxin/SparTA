    ...
    const int M, const int N, const int K, const int BLOCK_H, const int BLOCK_W)
{
    ...
    __shared__ half As[BK][BM + APAD];
    __shared__ half Bs[BK][BN + BPAD];
    __shared__ int local_k_offset[BK];


    wmma::fragment<wmma::matrix_a, 16, 16, 16, half, wmma::col_major> frag_a[4];
    ...
    const int A_THREAD_PER_ROW = BM / 8; // 1 float4 = 8 half
    ...
    int k_round = (k_nnz -1 + BK)/BK;
    for(int rid=0; rid<k_round; rid++){
        if(tid<min(BK, k_nnz-rid*BK)){
            local_k_offset[tid] = csr_col[index_start + rid * BK + tid];
        }
        __syncthreads();
        #pragma unroll
        for(int k=A_BLOCK_ROW_START; k<BK; k+=A_TILE_ROW_STRIDE){
            float4 tmp_float4={0};
            if(k < k_nnz - rid * BK)
                tmp_float4 = FETCH_FLOAT4(csr_val[index_start*BM*1+rid*BM*BK+k*BM+A_BLOCK_COL_START]);
            FETCH_FLOAT4(As[k][A_BLOCK_COL_START]) = tmp_float4;
        }
        #pragma unroll
        for(int k=B_BLOCK_ROW_START; k<BK; k+=B_TILE_ROW_STRIDE){
            float4 tmp_float4 = {0};
            if(k<k_nnz-rid*BK)
                tmp_float4 = FETCH_FLOAT4(B[OFFSET(local_k_offset[k], bx*BN + B_BLOCK_COL_START, N)]);            
            FETCH_FLOAT4(Bs[k][B_BLOCK_COL_START]) = tmp_float4; 
        }
        __syncthreads();
        #pragma unroll
        for(int k_step=0; k_step<BK/16; k_step++){
            wmma::load_matrix_sync(frag_a[k_step], &As[k_step*16][wy*16], BM + APAD);
...
int main(int argc, char*argv[])
{
    ...
    for(int run=0; run<n_iter; run++){
        HGEMM_K<<<gridDim, blockDim>>>(d_row, d_col, d_val, dB, dC, M, K, N, BLOCK_H, BLOCK_W);
     }
